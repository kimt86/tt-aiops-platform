...
**공차 레그 분해** — `leg decomposition` **(retire됨 · mig 0081)**
: 각 공차 레그를 30초 GPS 이동 세그먼트로 나눠 **움직임(drive_s)/정지(stop_s)** 를 잰 진단이었음(`learn_leg_decomp`). 비용 라벨(순수주행=구간시간)은 `learn_travel_sample` 빈트립으로 이관돼(mig 0080) 이 분해는 제거됨. 당시 규명한 것: 주행 구간 속도 ≈ 13km/h(움직인 순간만 보면 ~22.8km/h), "중앙값 6.9km/h"는 직선÷정지포함시간÷짧은레그 가중 착시. 같은 OD에서도 변동계수 0.758(±76%)로 대기 노이즈가 지배해 어떤 거리 모델도 이 정확도 바닥을 넘지 못합니다.
...
: 배차 2단계(최소비용 매칭)의 트럭→작업 이동비용 = free_in + 이동시간. 이동시간은 **순수주행 = 주행 구간 시간**(출발→작업지점 도달; 경로 정체 포함·핸드오버 대기 제외)으로, **추론 도로망 위 최단시간 경로를 실측 "실제시간 곡선"으로 변환**해 계산합니다(2026-07-02부터, `roadgraph.rs`의 RouteCost/CostCurve — `road_route_eval`에서 "이 경로시간이면 실제 보통 N초"를 구간별 실제 p50/p90 매듭으로 학습, 단조 잠금·짧은 경로 바닥 ~89초 유지. 처음의 "비율 곱" 보정은 비단조 역전·바닥 소실로 당일 교체). 라우팅 불능 쌍(~3%)만 L3 폴백 = 같은 표에서 배운 맨해튼 거리→실제 곡선(R과 같은 실측 눈금; 거리÷3.30 m/s 어림은 콜드스타트만). 예전 225m 격자 조회는 폐기(mig 0082). 실현시간(핸드오버 포함) `learn_travel_zone225`는 계속 갱신하나 비용엔 미사용(참조용).
...
**learn_leg_decomp** — `empty-leg decomposition` **(retire됨 · mig 0081)**
: 공차 레그별 drive_s/stop_s 모션 분해였음. 비용 라벨(순수주행=구간시간)은 `learn_travel_sample` 빈트립으로 이관돼(mig 0080) 이 표는 제거됨.

**learn_travel_zone225_drive (폐기)** — `dropped 2026-07-02 · mig 0082`
: 225m OD 격자별 구간시간 p50/p90 매트뷰였음 — 격자 인터림의 마지막 잔재. [2단계 이동비용](#pure-drive-cost)이 도로망 경로시간→실제 곡선으로 전환되며 **폐기**. 실현시간판 `learn_travel_zone225`는 참조용으로만 갱신됩니다.

**road_route_eval** — `route-vs-actual eval + calibration source`
: 실측 빈 트립마다 **실제 구간시간(라벨) vs 도로망 경로시간 vs 맨해튼 거리**를 나란히 쌓는 표(10분마다·21일 자가 백필). 두 용도: ① 상시 게이트(도로망이 좋아지는지 상관으로 추적), ② 2단계 비용의 **두 실측 곡선**(경로시간→실제 p50/p90 · 맨해튼→실제 — 구간별 실제값 매듭·단조·바닥 유지) 원천.
...
: GPS 궤적에서 매시 추론한 방향 도로망(2026-07-02 재구축: **7,189노드 — 작업지점 5,888곳 커넥터 포함**, 길 방향은 도로 단위 다수결). Rust 방향 Dijkstra 라우터 `crates/api/src/roadgraph.rs`가 이 그래프로 **배차 2단계 이동비용을 계산**합니다(경로 성공률 96.7%·작업지점 스냅 100%). 첫 판(264노드)은 스냅 30%로 게이트 탈락 → 재구축 후 채택.
