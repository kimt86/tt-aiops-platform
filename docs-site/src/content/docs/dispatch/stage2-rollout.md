...
- 추천이 **흔들리지 않습니다**(안정적). 이동시간은 실제 GPS 주행 기록으로 배운 값(격자 기반)을 씁니다.
...
:::tip[지금 가능 — 라이브 맵 권고 오버레이]
**라이브 맵**에서 **"AI 배차 권고"** 를 켜면, 추천이 **트럭에서 작업 지점으로 향하는 점선**으로
표시됩니다(초록=제때 도착, 주황=늦음, 라벨에 트럭→크레인·예상 분). **참고용일 뿐 실제 배차는
바뀌지 않습니다.** 이게 B단계의 첫 도구입니다 — 추천을 눈으로 보며 실제 결정과 비교하세요.
:::
