...
1. *각 트럭 × 각 작업* 에 대해 "**지금부터** 그 트럭이 그 작업 자리에 도착하기까지 걸리는 시간"을 표로 만듭니다. 이 비용 = **빌 때까지 남은 시간(free_in) + 작업 자리까지 빈 차로 가는 시간(이동)**.
   ...
   - **‘가는 시간(이동)’은 지금 ‘순수 주행시간’으로 봅니다** — 트럭이 실제로 굴러가는 시간만. 크레인 앞에서 줄 서서 기다리는 시간은 이 이동시간에 넣지 않고(줄서기는 트럭이 아니라 그 크레인의 성질이라) **1단계 쪽 신호로 따로 봅니다.** 이동시간 추정 방법은 아래 [도착 시간 추정](/kc/dispatch/stage2-design/#도착-시간-추정--터미널-지형--학습-모델) 참고.
...
| 필요한 것 | 상태 | 설명 |
|---|---|---|
| 트럭이 크레인까지 가는 **이동시간 예측** | ✅ 있음(잠정판) | 지금은 **순수 주행시간 격자**(225m OD별 실측 주행시간, 없으면 직선÷속도). 더 나은 학습 모델은 아직 미완 — 아래 참고 |
