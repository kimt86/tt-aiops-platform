...
**어떻게:**
1. *각 트럭 × 각 작업* 에 대해 "**지금부터** 그 트럭이 그 작업 자리에 도착하기까지 걸리는 시간"을 표로 만듭니다. 이 비용 = **빌 때까지 남은 시간(free_in) + 작업 자리까지 빈 차로 가는 시간(이동)**.
   - 지금 **노는 트럭**: 빌 때까지 시간이 0이므로 작업 자리까지 **가는 시간**만.
   - **곧 비는 트럭**: **빌 때까지 남은 시간 + 작업 자리까지 가는 시간**.
   - **‘가는 시간(이동)’은 지금 ‘순수 주행시간’으로 봅니다** — 트럭이 움직이기 시작해 작업 자리에 도달할 때까지의 시간(경로에서 신호·정체로 잠깐 멈춘 것도 포함, 도착 후 크레인과 주고받는 대기만 제외). 크레인 앞에서 줄 서서 기다리는 시간은 도착 *이후*(크레인이 언제 드느냐)에 생기므로 이 이동시간에 넣지 않습니다. 이동시간 추정 방법은 아래 [도착 시간 추정](/kc/dispatch/stage2-design/#도착-시간-추정--터미널-지형--학습-모델) 참고.
...
| 필요한 것 | 상태 | 설명 |
|---|---|---|
| 트럭이 크레인까지 가는 **이동시간 예측** | ✅ 있음 | **추론 도로망 위 길찾기 시간 → 실측 "실제시간 곡선" 변환**(2026-07-02부터; 라우팅 안 되는 소수 쌍은 맨해튼 거리→실제 곡선). 예전 225m 격자표는 폐기 |
...
| **배가 떠나는 시간**(출항·작업완료 예정) | ✅ 있음(적재 중) | 선박 항차 표(시스템 이름 `VSB_VOYAGE`)에 출항·작업완료·접안·화물마감 예정시각이 다 있고, 우리 데이터로 **주기 적재 중**(`live_vessel_schedule`) |
