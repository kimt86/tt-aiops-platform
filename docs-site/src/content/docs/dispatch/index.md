...
:::note[지금 상태]
**1단계(마감 정하기) 그림자 구현됨 (2026-06-18).** 선박 출항 시각을 가져와, 각 QC가 남은 일을 출항 전에 끝낼 수 있는지 "여유"와 베이별 마감을 계산해 **TT Dispatch의 QC 작업 현황 카드에 표시**합니다(아직 실제 배차엔 연결 안 함 = 그림자). 다음은 정밀화(크레인별 속도·트윈) → 2단계(어느 트럭을 보낼지 매칭).
:::
