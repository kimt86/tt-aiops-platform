...
| **담당** | **순수 주행 성분(정지 제외, ~22.8km/h) = 2단계 배차 비용**(누가 제일 빨리 도착하나). 도착지 진입·크레인 대기는 **1단계 크레인 신호로 분리**. 실측 door-to-door 전체는 참고용 |
...
:::note[비용 설계 — 2단계는 순수주행, 진입정지는 1단계로 분리 (현행)]
공차 leg 시간 = **순수 주행(달림, ~22.8km/h)** + **도착지 진입·크레인 대기(중앙 ~67초, 크레인별 22~194초)**. 측정해 보니(`learn_leg_decomp`) 진입대기는 *어느 트럭이 오느냐*가 아니라 *그 크레인이 얼마나 막혔나*에 좌우됩니다(크레인별 ~9배 차이, 붐비면 ~2.5배). 그래서:
- **2단계 비용 = 순수 주행만**(`learn_travel_zone225_drive`) — 트럭별로 깨끗한 "누가 제일 빨리 물리적으로 도착하나".
- **진입대기 = 크레인별 1단계 신호**(`learn_crane_approach`) — "이 크레인 지금 밀렸나"를 1단계(마감·수급)가 봄.
...
|---|---|---|---|---|
| ① 이동시간 | OD존+거리/날씨 | 이동시간 분포(+주행/정지 분해) | **순수주행=2단계 비용** / 진입정지=1단계 크레인 신호 | 실제 trip 시간 |
...
| 공차 leg 분해 | 사이클 leg를 GPS 30초 구간분석 | 비용=순수주행 산정·진입대기 분리 | 공차 leg 1건을 주행초(달림 ~53%)/정지초(진입·큐 ~47%)/진입(arrived−gps_arrived 중앙 ~67초)로 쪼갬 (`learn_leg_decomp`) |
