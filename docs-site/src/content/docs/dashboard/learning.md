...
| **담당** | **순수주행 = 주행 구간 시간(출발→작업지점 도달, ~13km/h) = 2단계 배차 비용**(누가 제일 빨리 도착하나). 경로 정체는 포함, 도착 후 크레인과 주고받는 대기만 제외. door-to-door 전체(핸드오버 포함)는 참고용 |
...
**2단계 비용 = 순수주행 = 주행 구간 시간**(출발→작업지점 도달, `learn_travel_zone225_drive`) — 트럭별로 "누가 제일 빨리 물리적으로 도착하나". 경로에서 신호·정체로 멈춘 것은 *아직 주행 중*이라 포함하고, 도착해서 크레인과 주고받으려 기다리는 시간만 뺍니다.
...
| **출력** | 방향성 도로 그래프(노드·엣지) · 통행 방향 · 평균 속도 |
| **담당** | **(향후) 이동시간 모델의 '경로 거리' 피처**. 지금은 배차 비용에 쓰지 않음 — 아래 [현재 미연결] 참고 |
| **어떻게 학습?** | GPS 자취를 스켈레톤화해 노드·엣지로 잇고(매시간 cron), 각 엣지를 학습된 차선 방향장에 맞춰 일방/속도 부여. 방향성 다익스트라 라우터 존재(`road_node`/`road_edge`, `roadgraph.rs`) |
| **테스트/결과** | 📈 노드·엣지 수↑ = 지도 완성. 🧪 라이브맵에서 **화살표**로 방향 표시. 단 검증 결과 아직 비용에 못 씀(아래) |

:::caution[현재 미연결 — 도로 라우팅이 맨해튼을 못 이김]
격자는 좁은 다리·일방차선을 표현 못 해(틈 건너 두 점을 "인접"이라 함) 도로 그래프로 넘어가려 했습니다. 그래서 라우팅을 만들고(방향 다익스트라) **검증**했는데(스냅된 585 leg, 라벨=주행초): 도로 경로 상관 **0.490** < 맨해튼 **0.565** — 도로가 **더 나빴고**, 게다가 작업점이 도로망에서 중앙 ~62m 떨어져 있어(블록·안벽 안까지 길이 안 들어옴) **30%만 스냅**됐습니다. → **아직 비용에 연결하지 않음**(비용은 순수주행 격자 유지). 인프라는 더 촘촘한 그래프를 위해 보존.
:::
...
|---|---|---|---|---|
| ① 이동시간 | OD존+거리/날씨 | 이동시간 분포 | **순수주행(구간시간)=2단계 비용** (크레인 진입 대기는 폐기·측정≈0) | 실제 trip 시간 |
| ② 작업점 좌표 | 도착 GPS | 블록·크레인 좌표 | 존·거리 계산의 위치 앵커 | 새 GPS 잔차 |
| ③ 주행 차선 | 이동 GPS(스켈레톤) | 방향 도로그래프(노드·엣지) | (향후) 이동시간 모델의 경로거리 피처 — 지금은 미연결 | 실제 트럭 흐름 |
...
| 공차 leg 분해 | 사이클 leg를 GPS 30초 구간분석 | 진단용(주행 vs 정지 분해) | 공차 leg 1건을 구간시간과 그 안의 주행초(달림 ~53%)/정지초(~47%)로 쪼갬 (`learn_leg_decomp`). 비용 라벨은 구간시간 |
