...
description: 러닝 센터의 두 탭(예측 모델 7개 · 수집 데이터 16종) — 각 모델이 무엇을 입력받아 무엇을 내고 배차의 어느 부분을 맡는지, 어떻게 채점하는지, 그리고 우리가 어떤 데이터를 어디서 얼마나 모으는지 — 중·고등학생도 이해하도록.
sidebar:
  order: 5
---
...
| 탭 | 무엇을 보여주나 |
|---|---|
| 🧠 **예측 모델** | 우리가 쓰는 예측 모델 7개. 카드마다 **입력 → 출력 → 담당(배차의 어느 부분)** + 자기채점(검증) + 학습 추이 |
...
| **담당** | **2단계 배차 비용행렬의 '공차 도착시간'** — 어느 트럭을 보내야 가장 적게 비어 달리나의 핵심 입력 |
...
## 카드 ② — 순수 주행시간 (멈춤·줄서기 뺀 '진짜 달린' 시간)

| 항목 | 내용 |
|---|---|
| **입력** | 3초 고빈도 GPS 자취에서 **정지·큐 구간을 뺀** 순수 주행 leg |
| **출력** | 순수 주행시간(초) + **GPS로 추론한 도로망 그래프** |
| **담당** | 이동시간 예측의 **상한 정확도 검증** + 방향성 도로 **경로 라우팅**의 기반 |
| **어떻게 학습?** | 매시간 자동으로 학습·테스트(예측 vs 실측 leg)하고 데이터 누적량을 기록 |
| **테스트/결과** | 학습 OD 모델 오차율(MAPE)을 **맨해튼(직선격자) 기준선**과 비교. 3초 GPS·순수주행 표본이 쌓이며 커버리지·정확도 개선 |

> 멈춰 선 시간(크레인 줄·신호)은 "길이 얼마나 걸리나"와 무관하므로 빼고 봅니다. 이렇게 하면 **길 자체의 빠르기**를 깨끗하게 잴 수 있습니다.
...
|---|---|---|---|---|
| ① 이동시간 | OD존+거리/날씨 | 이동시간 분포 | 2단계 비용행렬의 공차 도착시간 | 실제 trip 시간 |
| ② 순수 주행시간 | 3초 GPS(정지 제외) | 순수 주행초+도로망 | 라우팅 기반·상한 검증 | 실측 주행 leg |
...
| 데이터 | 출처 | 쓰임 | 설명 |
|---|---|---|---|
| 3초 고빈도 GPS | GPS 웹소켓(이동 트럭만) | 도로망 추론·순수 주행시간 | 이동 트럭 위치를 3초마다. 도로 중심선 추론·정지 제외 주행의 원재료. 5일 보존 |
...
| 이동시간 표본(OD) | 사이클에서 수확(5분) | TT 이동시간 모델 | 출발존→도착존 leg의 실제 시간·거리·피처 |
| 순수 주행 표본 | 3초 GPS(정지 제외) | 순수 주행시간 모델 | 큐·정지를 뺀 순수 주행 leg. 상한 정확도 검증 |
