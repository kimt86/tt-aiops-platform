...
:::caution[K_QC_Q의 한계 — 해치커버를 유휴로 오계산할 수 있음]
K_QC_Q는 **LD/DS move 사이의 빈 갭을 전부 "트럭 대기 유휴"로** 셉니다. 그런데 **해치커버 작업**(QC가 선창 덮개를 여닫으러 이동)은 컨테이너 move 기록이 없어, 그 시간이 갭=유휴로 잡힙니다 — QC는 실제로 바쁜데도. 즉 K_QC_Q는 순수 "TT 대기"를 **과대평가**할 수 있습니다. MCH_OPERATION엔 해치커버 전용 작업코드가 없어(LD/DS/MO/MI/RH/GI/GO만) 코드로는 못 거릅니다. "QC가 TT만 기다린 시간"을 정확히 분리하려면 갭 동안 **트럭이 QC 밑에 있었는지를 websocket GPS로 교차확인**해야 합니다(트럭 있음=진짜 starvation · 없음=해치커버/정비 가능성). → 후속 조사 과제(MCH_OPER_JOBTYPE 분포 probe 필요).
:::
