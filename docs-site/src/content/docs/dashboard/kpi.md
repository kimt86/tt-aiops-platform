...
:::caution[K_QC_Q의 한계 — 절반 가까이가 '트럭 대기'가 아님 (TOS 실측)]
K_QC_Q는 **LD/DS move 사이 빈 갭을 전부 "트럭 대기 유휴"로** 셉니다. 그런데 QC엔 **LD/DS만 기록**되고(해치커버·베이 이동·기어 교체는 move 행 자체가 없음) 그 시간이 갭=유휴로 잡힙니다 — QC는 실제로 바쁜데도. **TOS 직접 조사(5일):** 갭을 `MCH_OPER_QUEUENAME`(베이 큐) 변경 여부로 나누니 **≈45%가 베이 변경**(QC가 다른 베이로 이동 = 갠트리 주행/해치, **트럭 대기 아님**), **≈55%만 같은 베이**(≈진짜 트럭 대기 + 베이 내 잡일). 즉 현재 K_QC_Q는 진짜 starvation을 **약 1.8배 과대평가**(베이변경 갭은 중앙 171s로 더 김). 갭 크기만으로는 깔끔히 안 갈라지지만(완만한 분포), **베이 변경 플래그가 좋은 분류기**. 정밀 지표(K_QC_TT_WAIT 후보) = **같은-베이 갭만** + (옵션) 갭 동안 **트럭이 QC 밑에 있었는지 GPS 교차확인**. (>30분 갭=식사/선박 간은 이미 제외.)
:::
...
K_RTG_Q는 안벽 QC가 아니라 **야드(RTG) 쪽 핸드오버 대기**입니다(기록의 크레인 ID가 RTG/ES). "QC가 트럭을 기다림"은 위의 **K_QC_Q**입니다. 둘은 다릅니다.
:::
