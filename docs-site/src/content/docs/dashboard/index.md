...
```
   ...
   (양하)▼  │ (적하)
      ┌──────────┐
      │ 🏗️ QC    │  안벽 크레인 — 배와 부두 사이에서 컨테이너를 들어올리는 거대한 팔
      └────┬─────┘
           │  🚚 TT(트럭)가 부두↔야적장을 왕복
           ▼
      ┌──────────┐
      │ 🏗️ RTG   │  야드 크레인 — 야적장에서 컨테이너를 쌓고 내림
      └────┬─────┘
           ▼
      🟦🟦🟦  야적장(Yard) — 컨테이너를 잠시 보관하는 블록들
```
...
```
빈 차 ─① 공차 주행─▶ 픽업지 ─② 받기─▶ (적재) ─③ 적재 주행─▶ 하역지 ─④ 넘기기─▶ 빈 차 …
       (배차 받고 빈 차로 감)   (크레인이 실어줌)        (짐 싣고 감)        (크레인이 받아감 = 유휴)
```
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 원천 ①  TOS Oracle  ("작업 일기장")                                       │
│   - 무슨 작업이 "있었다"는 공식 기록 (완료된 일의 역사)                     │
│   - 컨테이너·크레인·배·트럭의 작업 이벤트, 초 단위 시각                       │
│   - 오래된 건 지워짐(보존 ~15일/~35일)                                      │
└───────────────┬────────────────────────────────────────────────────────┘
                │ 추출기(extractor)가 정해진 시각에만 살짝 떠옴 (운영 DB 보호)
                ▼
        🗄️ Postgres (우리 창고)  ─── 대시보드/API는 여기"만" 읽음
                ▲
                │ API가 SSH 터널로 실시간 수신 (메모리에만, DB 저장 안 함)
┌───────────────┴────────────────────────────────────────────────────────┐
│ 원천 ②  websocket  ("실시간 GPS·크레인 신호")                             │
│   - 지금 이 순간 장비가 "어디서 무엇을 하는가"의 연속 관측                  │
│   - TT/RTG/QC ~450대의 GPS 위치 + 크레인 PLC(하중) 신호                    │
│   - 과거 없음 = 지금 순간만                                                 │
└────────────────────────────────────────────────────────────────────────┘
```
