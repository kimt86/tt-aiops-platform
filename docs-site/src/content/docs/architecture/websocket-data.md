...
## 6. 데이터 품질 감사 (2026-07-15)

실시간 시스템 전체가 이 피드에 얹혀 있어, 얼마나 안정적이고 커버리지가 좋은지 실측했다.

### 6.1 인프라·커버리지 — 매우 우수

| 항목 | 실측 |
| --- | --- |
| 갱신 간격 | **정확히 30초** (차량별 median·p90 모두 30s) |
| 리커넥트 | **0회** · SSH 터널 **4주+ 무재시작** |
| 메시지율 | ~965/분 (안정) · 24시간 차량수 441~501대 (피드 끊김 0) |
| 야드트랙터 커버리지 | **98.4%** — TOS가 DS/LD에 쓴 트럭 495대 중 **8대만 미관측** (오히려 우리가 더 많이 봄: 539대) |
| 필드 완전성 | 위치 100% · jobtype·topos1 88%(나머지는 latched로 보완) · container1·arrival은 결측이 아니라 상태 반영(적재/도착한 차량만) |

> "TOS 5,575대 vs 웹소켓 539대"로 놀랄 수 있으나 — 5,033대는 **외부 게이트 트럭**(GI/GO/GC 이동, 우리 야드트랙터가 아님)이다. 우리 차량 커버리지는 거의 완전하다.

### 6.2 ★ 근본 한계 — "정지하면 GPS 단말이 침묵한다"

유일한 한계이자 가장 중요한 발견:

- 전체 갱신 간격의 **4%가 5분 넘는 큰 끊김**(median 11분). 이 끊김의 **68%가 트럭이 짐을 싣고 작업하는 중**(운반·RTG 대기·곧 빔)에 발생.
- 규명: **차량별 균등**(불량 단말 아님) · **시간대 균등**(피드 혼잡 아님) · **작업지점 근처에 안 몰림**(크레인 구조물 차폐 아님, 야드 전역) · 끊김 동안 트럭은 거의 정지(11분에 185m).
- **결정타: 큰 끊김의 100%가 트럭이 멈춘 직후 시작된다**(끊김 직전 이동량 median 0m). 일반 간격은 52%만 정지(median 12m 이동).

즉 **차량 GPS 단말이 "움직이면 보고, 멈추면 침묵"** 하는 이동 기반 보고 방식이다. 우리 피드나 구조물 문제가 아니라 **소스 단말의 성질**이다.

### 6.3 이것이 설명하는 것

트럭은 **멈추면 보이지 않는다** — 그런데 우리가 가장 관측하고 싶은 건 **멈춰서 크레인을 기다리는 그 순간**이다. 이 하나의 성질이 여러 난제를 설명한다:

- **곧-유휴 시각(리드타임) 예측이 근본적으로 불가**한 이유 — [ADR 0002](/kc/decisions/0002-idle-leadtime-limits/) 참조.
- 부하 도착 시점이 흐릿했던 이유 (도착 = 정지 = 침묵).
- DS 물림 탐지 재현율이 낮은 이유 (핸드오버 = 정지 = 침묵).

### 6.4 고칠 수 있나

- **우리 쪽에선 불가** — 단말의 보고 정책(상류)이다. 피드는 단말이 보내는 것을 충실히 전달할 뿐.
- **진짜 해결** = 터미널 측 텔레매틱스 재설정(정지 중에도 주기적으로 보고) — 하드웨어/정책 협의 사안.
- **우리 완화책**(latched 필드 · 지오펜스 도착 · 맵매칭)은 정확히 이 "정지 중 침묵"을 메우려던 장치다 — 왜 필요했는지가 이 감사로 분명해졌다.
