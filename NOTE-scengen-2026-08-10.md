# scengen — qc_move_log 시각 의미 관련 기록 (2026-08-10, 추출기 트랙)

★같은 날 오후 사용자 지시로 scengen 담당이 이 트랙으로 이관되어 **§1 을 직접 수리했다**
(assemble.rs vessels[]: start_ts/service_s 를 들어올림 단위 추정 시작으로 — 실측 서비스
p50 양하 735→95s·적하 1,646→109s). §2 는 데이터 갱신이라 코드 변경 불요. 이 파일은
경위 기록으로 남긴다.
