...
MIN_OBJ = 80        # remove road blobs smaller than this (px)

trucks = {}
for r in csv.reader(open('/tmp/gps_moving.tsv'), delimiter='\t'):
    if len(r) < 4: continue
...
ax.axis('off')
plt.savefig('/tmp/gps_skeleton.png', bbox_inches='tight', pad_inches=0.05)
...
np.savez_compressed('/tmp/road_raster.npz', dens=dens, mask=mask, skel=skel,
