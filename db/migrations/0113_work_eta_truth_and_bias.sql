-- ⚠⚠ 이 마이그레이션의 **정답지 선택은 틀렸다**. qc_move_log.st_ts 를 '크레인 실제 시작'으로
--    읽었으나 그것은 배차 시각이다. db/migrations/0115_work_eta_truth_is_comp_ts.sql 이 정정한다.
--    아래 본문의 실측치(DS −35s / LD +391s 등)는 전부 철회 대상이니 인용하지 말 것.
--    이 파일은 이력이라 본문은 그대로 둔다.
--
