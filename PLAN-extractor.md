...
| 청크 | 상태 | 커밋 |
|---|---|---|
| 1 부하장부 + 타이머 분산 | **완료·라이브** | `5ea3909` |
| 2 KPI 병산 | **완료·라이브** | `ebb3249` |
| 3 적부계획 델타 | **완료·라이브** | `1cfeaf3` |
| 4 스트림 확장 + scengen 로컬화 | **완료·라이브** | `c2426c2` |
| 5 workpool 델타 | **철회**(아래) | — |
| 6 KPI 절체 + nightly 로컬화 | **완료·라이브** | `aa23311` |
| 7 workpool 왕복 감축 + K_EMPTY | **완료·라이브** | `aa074b0` |
| 7-3 K_CYCLE 재정의 | **철회**(아래) | `5741c1e` |

**최종 실측**: Oracle 왕복 522 → **336회/h**. 하루치 재스캔 60회/h → 0. t1 틱 8.4초 → 0.25초.
적부계획 전송 74,400행/h → 930행/h. workpool 틱 Oracle 11초 → 6.7초.
검증 시점 전 유닛 success, 배차 신선도 30초, `live_assigned_tt` 트럭 83대 = Oracle 83대.

### 철회 2건 (되살리려면 먼저 읽어라)

**CHUNK 5 (workpool 델타)** — 전송은 −83~93% 나왔으나 라이브에서 드리프트가 40분에 106행까지
자라고 치유 직후 한 틱에 62행이 증발. 원인 유력: 병합 키 `(contno,jobtype)` 가 유일하지 않다
(재발행으로 같은 상자·같은 유형의 열린 지시가 2개 존재 가능). 코드·인덱스·잔재 전부 제거함.
★애초에 **방향이 틀렸다**: 전체 SELECT 2.61초 vs COUNT(*) 2.01초 = 왕복 고정비가 지배하므로
행을 줄여봐야 이득이 없다. CHUNK 7 의 왕복 감축이 같은 목적을 위험 없이 달성했다.

**7-3 (K_CYCLE 재정의)** — 표시 K_CYCLE = c10 정의가 **의도된 설계**다(`api/agg.rs:82`
"(raw_k_cycle) kept internally, not displayed" / `shift.rs:414` "Displayed K_CYCLE is the REAL
TT cycle"). 오케스트레이터가 인용한 "41% 과소"는 **다른 화면**(`api/cycles.rs` v1) 메모였다.
강행하니 `/api/live` 1200.6 vs `/api/kpis` 802.0 으로 같은 카드가 갈렸다 → 전량 원복.
다시 하려면 **crates/api 담당과 함께**(표시 경로가 두 곳이라 동시 수정 필요).

### 다음 액션 — ✅완료 (2026-08-10 K_EMPTY 병산 대조 + 산식 확정)

08-09 하루 양쪽 재계산: oracle 비율 0.42~0.52·공차 1,142~1,607m vs local 0.32~0.47·819~1,134m.
차이는 산술이 아니라 모집단·산식(전부 실측): ①e4는 JOBSTATUS 무필터 — 미완료·재발행 작업
계수(2h 창 분해에서 35%가 완료행 없음) ②UN_LNDN이 전이행마다 다른 작업 45% — e4의 MAX가
완료행 대비 +33%(946→1,262) ③e4는 MO 포함, 로컬 소스(tos_handover_label)는 DS/LD만.
로컬 값=완료행 최종값과 일치(거울 정확).

★사용자 결정(2026-08-10): **a안 확정 — 로컬 정의 유지, MI/MO는 지표에서 제외**("신경 안 씀").
Oracle e4는 킬스위치(`KPI_NIGHTLY_SRC=oracle`)로만 존치. 절체 전(08-08 이전) raw_k_empty는
옛 산식 값 그대로 보존 — 기간 조회가 08-08을 걸치면 단차가 섞인다(KC kpi 문서 2곳에 고지 완료:
`kc/dashboard/kpi.html`·`kc/data/kpi-computation.html`).

### 아직 Oracle 을 쓰는 KPI 경로 (의도된 잔존)
- `VOYAGE_PLAN`(VSS_STATISTICS) — t2 15분. 로컬 대체 재료 없음.
- `k_cycle`(e3b → raw_k_cycle) — nightly 1회. 로컬화 = 재정의라 보류(위 7-3 참조).
