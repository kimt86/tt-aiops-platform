...
import { api, type LearnTopos, type LearnToposPoint, type LanesData, type TravelData, type TravelOd, type SoonIdleData, type SoonIdleLead, type DispatchPredData, type DataStat, type DataRow } from "./api";
    ...
    <>
      ...
      <Session n={1} accent="#60a5fa" title={k ? "TT 이동시간 예측" : "TT travel time"} sub={k ? "사이클에서 수확한 출발→도착 trip + 피처(경로거리·존·밀도·날씨)" : "trips harvested from cycles + features"}>
        ...
        <div className="ls-chips">
          ...
          <Chip label={k ? "구간 속도 (출발→도달)" : "segment speed"} value={tv?.median_speed_kmh != null ? `${kmh(tv.median_speed_kmh)} km/h` : "—"} accent="#60a5fa" />
        </div>
        <div className="ls-note">{k ? `비용 라벨 = 순수주행(주행 구간 시간, 출발→작업지점 도달). 경로에서 신호·정체로 멈춘 것은 아직 주행 중이라 포함하고, 도착 후 크레인과 주고받는 대기만 뺍니다. 큰 크레인 대기는 ARRIVED 이후라 이동시간에 안 들어감. 같은 OD의 변동(±76%)은 야드 확률성 — 점예측보다 분포로 사용.` : `Cost label = 순수주행 (drive-segment time, start→work point). En-route stops are still driving and stay in; only the post-arrival handover wait is excluded. The crane queue is after ARRIVED (not in travel time). Within-OD variance (±76%) — use as a distribution.`}</div>
      ...
      {/* ③ 주행 차선 */}
      <Session n={3} accent="#34d399" title={k ? "주행 차선·도로 방향" : "Driving lanes & direction"} sub={k ? "이동 TT의 GPS 트레이스를 22m 격자에 집계 → 도로·방향" : "moving-TT traces aggregated to a 22m grid → roads & direction"}>
      ...
      {/* ④ Soon-idle 예측 정확도 */}
      <Session n={4} accent="#a78bfa" title={k ? "Soon-idle (곧 빔) 예측" : "Soon-idle prediction"} sub={k ? "그림자: 예측 vs 실제 트럭 빔 — GPS 우선(사이클 dropped_at)·TOS 폴백 · DS·LD" : "shadow: prediction vs physical truck-freed — GPS-first (cycle dropped_at), TOS fallback"}>
      ...
      <Session n={5} accent="#fb923c" title={k ? "유휴 분 예측 — 피처 정밀화 (DS·LD)" : "minutes-to-idle — feature refinement (DS·LD)"} sub={k ? "'몇 분 후 유휴'를 피처로 더 맞출 수 있나 — DS=RTG거리·신호 / LD=안벽이라 피처 없음" : "can features sharpen 'minutes-to-idle' — DS uses RTG distance×signal; LD is quay-side"}>
      ...
      {/* ⑥ 배차 작업시점 예측 (1단계) */}
      <Session n={6} accent="#f472b6" title={k ? "배차 작업시점 예측 (1단계)" : "Dispatch work-time prediction (Stage 1)"} sub={k ? "출항 역산 + 학습한 크레인 속도로 '크레인이 각 컨테이너를 작업할 시각'을 예측 → 배차 마감 산정" : "vessel-departure backsolve + learned crane pace → per-container work time → dispatch deadline"}>
