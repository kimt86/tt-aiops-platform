...
## 인계 — 이 세션은 2026-08-21 11:40 KST 에 손을 뗐다 (사용자 결정)

**왜**: 10:19 이후 **다른 세션이 같은 파일(`crates/api/src/livemap.rs`)과 같은 라이브 서비스(tt-api)를 동시에
수정**하고 있다(커밋 `e8412a8` · 11:24 재빌드·재시작 — 이 세션이 하지 않았다). 두 세션이 번갈아 재시작하면
`acb8677` 유형의 사고가 난다. 사용자 결정으로 **그쪽 세션이 마무리한다.**

### 판별자별 풀 재현율 (같은 잣대·`assigned_tt_hist` 기준·150초 창) — 이어받는 쪽이 쓸 것

| pool_ver | 무엇 | 창(MYT) | 요청 | 재현율 |
|---|---|---|---|---|
| 1 | 첫 배포 | 08-19 12:57~14:09 | 1,201 | 54.5% |
| 2 | 픽업 가드 + 앵커 status 필터 제거 | 08-19 14:10~08-21 07:59 | 34,656 | **98.7%** |
| 3 | 리뷰 반영(적하 앵커 통째 제거 포함) | 08-21 08:00~09:19 | 949 | 87.6% ← 회귀 |
| 4 | 적하 앵커를 '값'에서만 미룸(소속 유지) | 09:20~10:24 | 1,024 | 94.8% ← 기준 미달 |
| 5 | 위치 상한 3600→10800s (다른 세션) | 10:25~ | 220 | **98.2%** |

- **ver3 의 교훈(지우지 말 것)**: 적하에서 앵커를 버릴 때 *값*만이 아니라 *풀 소속*까지 버려 98.7→87.7% 로
  떨어졌다. `livemap.rs:4711` 주석이 이미 "GPS 에 없는 것은 정확도가 아니라 **커버리지**"라고 적어뒀는데
  그 구분을 지운 것이 원인이었다. **정확도 규칙과 소속 규칙을 같은 분기에서 처리하지 말 것.**
- **ver4→5 의 교훈**: 위치 상한 3600s 가 재현율을 94.8% 로 눌렀다. 다른 세션의 근거(명단 판정이 'TOS 활동
  ≤3h' 이니 위치도 같은 창)가 임의 튜닝이 아니라 정합이라 타당하다고 본다.
- **아직 안 된 것**: ver5 의 **6시간 확정 측정**(지금은 220 요청·13분). 창이 짧아 확정으로 쓰면 안 된다.
  `scripts/pull_model_coverage.sql` 의 ⑮가 최신 판만 자동으로 잡는다.
- 1차 리뷰(code-reviewer) 지적 18건 중 **거절 2건은 그대로**: MI/MO 픽업 가드 사각지대(실측 0.06%),
  명단 규칙의 'GPS 30분' 갈래 미구현(없이도 목표 달성). **2차 리뷰는 돌리지 않았다.**

### 이 세션이 남긴 것 (커밋 `c90f114`~`2a72ce9`)

풀 재정의·mig 0154/0155/0156·측정 절 ⑫~⑰·`is_free_tos` 순수함수+테스트 6건(돌연변이 검증)·RETENTION 등록·
`DeviceOut.in_pool` 발행(프론트가 GPS 라벨로 후보를 재구성하던 것 교체)·KC 2단계 설계 갱신.

### ⚠ 범위 밖에서 발견한 것 (누구든 확인 필요)

- **시나리오 타이머 8개가 2026-08-19 15:57 KST 에 전부 정지**됐고 지금 `disabled·inactive` 다
  (`collect·contspec·enrich·gate·plan·yard·yard-build`, `assemble` 만 활성). 08-21 10:56~11:01 에 누군가
  수동으로 한 번 돌렸다. 이 세션이 한 일이 아니다. 의도한 정지가 아니면 시나리오 수집이 그 이후 멈춰 있다.
- `scenario/qc_plan_coverage` warn 은 상시 조건(7일 114항차 중 13건 `missed_preberth`·임계 missed>0)이고
  장애가 아니다. 같이 뜨는 "엇갈리는 큐 57개"는 임계 150 미만이라 트립 요인이 아니다.
